...
export default function WeekSchedule() {
  ...
  return (
    <ScrollView contentContainerStyle={[styles.container, { backgroundColor: colors.background }]}>
      <Text style={[styles.title, { color: colors.text }]}>Week Schedule: {weekStart}</Text>
      {sortedDayKeys.map((dayKey: string) => {
        const localDate = localDateFromString(dayKey);
        return (
          <TouchableOpacity
            key={dayKey}
            style={[styles.dayCard, { backgroundColor: colors.tint }]}
            onPress={() =>
              router.push(
          ...
          >
            <Text style={[styles.dayText, { color: colors.buttonText }]}>{localDate.toDateString()}</Text>
          </TouchableOpacity>
        );
      })}
      <TouchableOpacity onPress={() => router.back()} style={[styles.backButton, { backgroundColor: colors.buttonBackground }]}>
        <Text style={[styles.backButtonText, { color: colors.buttonText }]}>Back</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
