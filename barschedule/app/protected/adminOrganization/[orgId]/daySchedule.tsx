...
export default function DaySchedule() {
  ...
  //fetch employees so it is live coming from firestore
  React.useEffect(() => {
    const fetchEmployeesAndRoles = async () => {
      try {
        // 1. Fetch the Organization document to get roles
        ...
        setAllRoles(orgData?.roles || []); //save roles from org to allRoles state

        //fetch the Employees subcollection
        const employeeRef = collection(db, "Organizations", orgId, "Employees");
        const employeeDocs = await getDocs(employeeRef);
        //get employees and their roles
        const fetchedEmployees = employeeDocs.docs.map((doc: any) => ({
          userId: doc.id,
          name: doc.data().name,
          orgRoles: doc.data().orgRoles || [],
        }));
      ...
      {daySchedule.map((rs) => (
        <View key={rs.role} style={styles.roleSection}>
          ...
          {rs.shifts.map((shift, shiftIndex) => {
            const employeesForThisRole = employees.filter(emp => emp.orgRoles?.includes(rs.role));
